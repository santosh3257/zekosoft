...
class User extends BaseModel
{
    ...
    public static function allMyClients($exceptId = null, $active = false, $companyId = null)
    {
        ...
        $clients = User::with('clientDetails')
            ...
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->join('client_details', 'users.id', '=', 'client_details.user_id')
            ->select('users.id', 'users.name', 'users.email', 'users.created_at', 'client_details.company_name', 'users.image', 'users.email_notifications', 'users.mobile', 'users.country_id','client_details.company_name','client_details.social_security','client_details.account_type','client_details.address','client_details.city','client_details.state','client_details.postal_code','client_details.website','client_details.office_phone','client_details.gst_number')
            ->where('roles.name', 'client')->whereNull('users.deleted_at');
    ...
    public static function allMyClientsWithPaginate($client_name, $account_type, $co_applicant, $status, $per_page)
    {
        ...
        $clients = User::with('clientDetails')
            ...
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->join('client_details', 'users.id', '=', 'client_details.user_id')
            ->select('users.id', 'users.email','users.name', 'users.created_at', 'client_details.company_name', 'users.image', 'users.email_notifications', 'users.country_id','users.status','client_details.account_type','client_details.co_applicant_name')
            ->where('roles.name', 'client')->whereNull('users.deleted_at');
        ...
        if (!empty($client_name)) {
            ...
            $clients->where('users.name', 'like', '%' . $client_name . '%')->orWhere('client_details.company_name', 'like', '%' . $client_name . '%');
        }

        if (!empty($co_applicant)) {

            $clients->where('client_details.co_applicant_name', 'like', '%' . $co_applicant . '%');
        }
        ...
        if (!empty($account_type)) {
            ...
            $clients->where('client_details.account_type', $account_type);
        }

        if (!empty($status)) {

            $clients->where('users.status', $status);
        }
        // if (!is_null($companyId)) {
        //     $clients->where('users.company_id', '<>', $companyId);
        // }
        ...
        return $clients->orderBy('users.name', 'asc')->paginate($per_page);
    }
    ...
    {
        return User::with('clientDetails')
        ->join('role_user', 'role_user.user_id', '=', 'users.id')
        ->join('roles', 'roles.id', '=', 'role_user.role_id')
        ->join('client_details', 'users.id', '=', 'client_details.user_id')
        ->where('roles.name', 'client')
    ...
    public static function previousWeekCustomerCount()
    {
        ...
        ->join('roles', 'roles.id', '=', 'role_user.role_id')
        ->join('client_details', 'users.id', '=', 'client_details.user_id')
        ->select('*')->whereBetween('users.created_at', [Carbon::now()->subWeek()->startOfWeek(), Carbon::now()->subWeek()->endOfWeek()])
        ->where('roles.name', 'client')
        ->whereNull('users.deleted_at')
        ->get()->count();
    }
    public static function currentWeekCustomerCount()
    {
        Carbon::setWeekStartsAt(Carbon::SUNDAY);
        Carbon::setWeekEndsAt(Carbon::SATURDAY);
        return User::with('clientDetails')
        ...
        ->join('roles', 'roles.id', '=', 'role_user.role_id')
        ->join('client_details', 'users.id', '=', 'client_details.user_id')
        ->select('*')->whereBetween('users.created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
        ->where('roles.name', 'client')
        ->whereNull('users.deleted_at')
        ->get()->count();
    }
    ...
    public static function allCompanyEmployee(){

        $users = User::withRole('employee')
            ->select('users.id', 'users.company_id', 'users.name', 'users.created_at', 'users.image');
