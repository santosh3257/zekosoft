...
class NewInvoice extends BaseNotification
{
    ...
    public function __construct(Invoice $invoice)
    {
        ...
        $this->user = User::findOrFail($invoice->client_id);
    ...
    public function toMail($notifiable)
    {
        ...
        $build = parent::build();
        $content = __('email.invoice.text');
        $invoice = Invoice::with('houseService','items.product','items.houseWork','items.TaxInfo','items.accountCode')->where('id',$this->invoice->id)->first();
        $pdf = PDF::loadView('invoices.pdf.invoice_pdf_download', compact('invoice'));
        $orderPdfLink = 'invoice'.$this->invoice->id.'.pdf';
        ...
        $url = url('user-uploads/invoice/'.$orderPdfLink);
            $build->subject(__('email.invoice.subject') . ' - ' . config('app.name') . '.')
            ->markdown('mail.email', [
                'content' => $content,
                'notifiableName' => $notifiable->name
            ]);
            $build->attachData($pdf->output(), $orderPdfLink);
            return $build;
    }
