...
class PackageDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('monthly_price', function ($row) {
                return ($row->default === 'no' && $row->monthly_status == '1') ? global_currency_format($row->monthly_price, $row->currency_id) : '--';
            })
            ->editColumn('annual_price', function ($row) {
                return ($row->default === 'no' && $row->annual_status == '1') ? global_currency_format($row->annual_price, $row->currency_id) : '--';
            })
            ->editColumn('max_project', function ($row) {
                return ($row->project_unlimited == 'true') ? 'Unlimited' : $row->max_project;
            })
            ->editColumn('max_employees', function ($row) {
                return ($row->employee_unlimited == 'true') ? 'Unlimited' : $row->max_employees;
            })
    ...
    protected function getColumns()
    {
        return [
            ...
            __('superadmin.max') . ' ' . __('superadmin.projects') => ['data' => 'max_project', 'name' => 'max_project', 'title' => __('superadmin.max') . ' ' . __('superadmin.projects')],
