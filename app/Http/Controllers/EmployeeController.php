...
class EmployeeController extends AccountBaseController
{
    ...
    public function applyQuickAction(Request $request)
    {
        ...
        case 'change-status':
            ...
            if ($request->status == 'active' && !is_null($company->employees) && $company->employees->count() >= $company->package->max_employees) {
                return Reply::error(__('superadmin.maxEmployeesLimitReached'));
            }
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        if (request()->has('status')) {
            $company = Company::with(['package', 'employees' => function ($q) {
                $q->withoutGlobalScope(ActiveScope::class);
            }])->where('id', user()->company_id)->first();

            if (request()->status == 'active' && !is_null($company->employees) && $company->employees->count() > $company->package->max_employees && $user->status != 'active') {
                return Reply::error(__('superadmin.maxEmployeesLimitReached'));
            }
