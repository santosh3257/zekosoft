...
<div class="modal-body">
    <x-form id="createTax">
        ...
        <div class="row">
            <div class="col-sm-12">
                <x-forms.select fieldId="vat_type_id" :fieldLabel="__('modules.invoices.vatType')" fieldName="vat_type_id" fieldRequired="false">
                    <option value="">Select Type</option>
                    @if($vatType)
                    @foreach ($vatType as $type)
                        <option value="{{ $type->id }}">{{ $type->vat_type }}</option>
                    @endforeach
                    @endif
                </x-forms.select>
            </div>
