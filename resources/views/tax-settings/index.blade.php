                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-2">
                            <!-- <x-forms.button-primary icon="plus" id="add-tax" class="type-btn mb-2 actionBtn">
                                @lang('modules.credit-notes.addVatPercentage')
                            </x-forms.button-primary> -->
                            <x-forms.button-primary icon="plus" id="add-taxType" class="type-btn mb-2 actionBtn">
                                @lang('modules.invoices.vatType')
                            </x-forms.button-primary>
                        </div>
            ...
            <div class="col-lg-12 col-md-12 ntfcn-tab-content-left w-100 p-0">
                <div class="table-responsive">
                    <x-table class="table-bordered table-hover">
                        <x-slot name="thead">
                            <th>#</th>
                            <th>@lang('modules.invoices.vatType')</th>
                        ...
                        @forelse($taxes as $key => $tax)
                            <tr id="tax-{{ $tax->id }}">
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $tax->vat_type }}</td>
                                <td>@if(!empty($tax->vatPercentage)){{ $tax->vatPercentage->percentage }}% @endif</td>
...
@push('scripts')
    ...
    <script>
        ...
        // create new tax
        $('#add-taxType').click(function () {
            const url = "{{ route('vat-types.create') }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
        ...
        $('body').on('click', '.delete-tax', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let url = "{{ route('vat-types.destroy', ':id') }}";
                    url = url.replace(':id', id);
