    ...
    public function up(): void
    {
        ...
        if (!$checkFirstExists) {
            ...
            if (is_null($superadminRole)) {
                ...
                $permissions = Permission::select('permissions.*')->join('modules', 'modules.id', 'permissions.module_id')->where('modules.is_superadmin', 1)->get();

                $permissionRole = [];
                $userPermission = [];

                $superadmins = User::withoutGlobalScopes()->whereNull('company_id')->get();
